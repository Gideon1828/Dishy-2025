...
const VisitDish = () => {
    ...
    const fetchRatingsAndComments = async () => {
      try {
        ...
        const res = await fetch(
          `https://dishy-2g4s.onrender.com/rate-comment/${id}`,
          {
            headers: {
  ...
  const handleUpdateComment = async (commentId) => {
    try {
      ...
      const res = await fetch(
        `https://dishy-2g4s.onrender.com/api/comments/${commentId}`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
  ...
  const ratedUsers = ratings.filter(
    (r) => typeof r.rating === "number" && r.rating > 0
  );
  const averageRating = ratedUsers.length
    ? (
        ratedUsers.reduce((acc, r) => acc + r.rating, 0) / ratedUsers.length
      ).toFixed(1)
    : 0;
      ...
      <div className="dish-container">
        ...
        <div className="section">
          ...
          <div className="equipment-list">
            {dish.analyzedInstructions?.[0]?.steps
        ...
        <div className="feedback-section">
          ...
          <div className="average-rating-display" style={{ margin: "10px 0" }}>
            ...
            <span style={{ marginLeft: "8px" }}>
              {averageRating} ({ratedUsers.length} user
                ...
                {currentComments
                  ...
                  .map((cmt) => (
                    <div key={cmt._id} className="comment-box">
                      <p>
                        <strong>{cmt.username || "Anonymous"}:</strong>
                      </p>
                      {editingCommentId === cmt._id ? (
                        <>
                          ...
                          />
                          <button onClick={() => handleUpdateComment(cmt._id)}>
                            Save
                          </button>
                        ...
                        <>
                          ...
                          {currentUser?.id === cmt.userId && (
                            <div className="comment-actions">
                              <button
                                onClick={() =>
                                  handleEditClick(cmt._id, cmt.comment)
                                }
                              >
                                Edit
                              </button>
